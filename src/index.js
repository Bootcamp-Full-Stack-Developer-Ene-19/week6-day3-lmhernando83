...
const searchPokemonAsync = async () => {
    ...
    if(inputText.length <= 0) {
        ...
        const rawdata = await fetch(`${ENDPOINT}pokemon/${inputText}`);
...
const searchEvolutionsAsync = async () => {
    ...
    const chain = await checkEvolutionChain(evolution.chain.evolves_to);
    getEvolutionsAsync();
};

const getPokemonSpeciesAsync = async (species) => {
    const rawdata = await fetch(species.url);
    const data =  await rawdata.json();
    return data;
};

const getEvolutionChainAsync = async (chain) => {
    const rawdata = await fetch(chain.url);
    const data =  await rawdata.json();
    return data;
};

const getEvolutionsAsync = async () => {
    const rawdata = await fetch(chain.chain.evolves_to);
    const data =  await rawdata.json();
    return data;
};
